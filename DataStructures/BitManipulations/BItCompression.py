...
# There is an array
# A of N integers. He has to perform certain operations on the elements. In any one operation,
# two indices i and j (i < j) are chosen, and A[i] is replaced with A[i] & A[j],
# and A[j] is replaced with A[i] | A[j], where & represents
# the Bitwise AND operation and | represents the Bitwise OR operation. This operation is performed over all the pairs
