...
class Solution:
    ...
    def solve(self, A):
        ...
        heapq.heapify(A)
        while len(A) > 1:
            min_item_1 = heapq.heappop(A)
            min_item_2 = heapq.heappop(A)
            item_sum = min_item_1 + min_item_2
            ans += item_sum
            heapq.heappush(A, item_sum)
        return ans
...
# 2) Now connect ropes of lengths 8 and 10. Now, we have two ropes of length 18(8 + 10) and 10(4 + 6).
